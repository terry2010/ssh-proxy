...
# === 6. 编译检查 + 测试 ===
info "编译检查 ..."
if ! cargo check --workspace 2>&1 | tee /tmp/termfast-release-check.log | grep -q "Finished"; then
  cat /tmp/termfast-release-check.log | tail -20
  die "cargo check 失败，请修复后再发布"
fi
ok "cargo check 通过"

info "tsc 类型检查 ..."
if ! npx tsc --noEmit 2>&1 | tee /tmp/termfast-release-tsc.log; then
  die "tsc 类型检查失败"
fi
if grep -q "error" /tmp/termfast-release-tsc.log; then
  die "tsc 类型检查有错误"
fi
ok "tsc 类型检查通过"

info "前端单元测试 ..."
npm test -- --run 2>&1 | tee /tmp/termfast-release-test.log
if grep -q "passed" /tmp/termfast-release-test.log && ! grep -q "failed" /tmp/termfast-release-test.log; then
  ok "npm test 通过"
else
  die "npm test 失败"
fi
