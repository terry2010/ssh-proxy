...
export function ServerList({
  ...
  const blockHoverExpand = useCallback(() => {
  ...
  const handleExpandButtonMouseLeave = useCallback((e: React.MouseEvent) => {
    hoverBlockedRef.current = false;
    const related = e.relatedTarget as HTMLElement | null;
    if (sidebarRef.current && related && sidebarRef.current.contains(related)) {
      setHoverExpanded(true);
    }
  }, []);
      ...
      const openTerminal = async () => {
        ...
        if (!alreadyConnected) {
          ...
          try {
            ...
            if (
              err instanceof IpcErrorImpl &&
              err.code === "CredentialNotFound"
            ) {
  ...
  const abnormalStatuses: ServerStatus[] = [
    "auth_failed",
    "reconnecting",
    "offline",
  ];
  ...
  const showFullContent = !collapsed || hoverExpanded;
      ...
      <div
        ...
        className={`flex flex-col h-full transition-none ${
          collapsed && hoverExpanded
            ? "absolute left-0 top-0 w-64 h-full z-50 bg-[#FBFBFA] dark:bg-[#1E1E1E] border-r border-gray-200/80 dark:border-white/5 shadow-[inset_-2px_0_12px_rgba(0,0,0,0.05)] dark:shadow-[inset_-2px_0_12px_rgba(0,0,0,0.4)]"
            : collapsed
              ? "w-12 bg-[#FBFBFA] dark:bg-[#1E1E1E] border-r border-gray-200/80 dark:border-white/5 shadow-[inset_-2px_0_12px_rgba(0,0,0,0.05)] dark:shadow-[inset_-2px_0_12px_rgba(0,0,0,0.4)]"
              : "w-64 bg-[#FBFBFA] dark:bg-[#1E1E1E] border-r border-gray-200/80 dark:border-white/5 shadow-[inset_-2px_0_12px_rgba(0,0,0,0.05)] dark:shadow-[inset_-2px_0_12px_rgba(0,0,0,0.4)]"
        }`}
      >
        ...
        >
          ...
          {showFullContent ? (
            ...
            <button
              className="w-8 h-8 flex items-center justify-center rounded-lg bg-[#007AFF] text-white hover:bg-[#0063D1] transition-colors"
              onClick={() =>
                onAddServer ? onAddServer() : setShowAddDialog(true)
              }
              title={t("server.add")}
          ...
          {!collapsed && (
            ...
            >
              <svg
                ...
                strokeLinejoin="round"
              >
                <polyline points="14 17 8 12 14 7" />
              </svg>
            </button>
          )}
        </div>
...
function ServerListItem({
  ...
  return (
    <div
      className={`group relative flex items-center gap-3 cursor-pointer transition-all duration-150 rounded-lg mx-2 my-0.5 ${
        ...
        isDragOver && dragOverPos === "before"
          ? "border-t-2 border-[#007AFF]"
          : ""
      } ${isDragOver && dragOverPos === "after" ? "border-b-2 border-[#007AFF]" : ""}`}
      onClick={onSelect}
    ...
    >
      <div
        className={`w-2.5 h-2.5 flex-shrink-0 rounded-full ${selected ? "" : STATUS_COLORS[server.current_status]} ${STATUS_SHAPES[server.current_status]} ${selected ? "bg-white border-white" : ""}`}
        aria-hidden
      />
      {!collapsed && (
        <>
          <div className="flex-1 min-w-0">
            <div
              className={`text-sm font-medium truncate ${selected ? "text-white" : "text-gray-900 dark:text-gray-100"}`}
            >
              {server.name}
            </div>
            <div
              className={`text-xs truncate ${selected ? "text-white/80" : "text-gray-500 dark:text-gray-400"}`}
            >
