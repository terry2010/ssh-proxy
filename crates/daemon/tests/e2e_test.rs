...
async fn setup_daemon() -> (DaemonServer, std::path::PathBuf) {
    ...
    let server = DaemonServer::start_with_path(state, socket_path.into())
        .await
        .unwrap();
...
async fn test_e2e_log_buffer() {
    ...
    let entries = server
        .state()
        .log_buffer
        .get_entries(Some("srv_log_1"), None, None)
        .await;
